...
from processor.config.unit import UnitConfig
from processor.config.unit import \
...
def setup_loggers(verbose_level, processor):
    log_config = get_log_config(filename="unit_log_config.toml")

    # If no toml, try loading yaml
    if log_config is None:
        log_config = get_log_config(filename="unit_log_config.yaml")
    ...
    if log_config is not None:
        ...
        log_configuration(
            log_dir=log_dir,
            name="unit-" + str(processor.zmq_id)[2:-1])
...
def create_parser(prog_name):
    parser = argparse.ArgumentParser(
        prog=prog_name,
        description='Starts a hashblock-unit transaction processor.',
        epilog='This process is required to apply any changes to on-chain '
               'Units used in the Hashblock system.',
...
def load_settings_config(first_config):
    default_settings_config = \
        load_default_unit_config()
    conf_file = os.path.join(get_config_dir(), 'unit.toml')
...
def main(prog_name=os.path.basename(sys.argv[0]), args=None,
    ...
    my_logger.debug("Unit handler instantiated, starting processor thread...")
